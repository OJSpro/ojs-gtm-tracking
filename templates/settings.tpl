...
<form class="pkp_form" id="gtmSetupSettingsForm" method="post" action="{url router=\PKP\core\PKPApplication::ROUTE_COMPONENT component="grid.settings.plugins.SettingsPluginGridHandler" op="manage" verb="settings" plugin=$pluginName category="generic" save=true}">
	...
	{fbvFormArea id="gtmSettings"}
		...
		{fbvFormSection list=true}
			...
			{fbvElement type="checkbox" name="trackBackend" id="trackBackend" checked=$trackBackend label="plugins.generic.gtmSetup.manager.settings.trackBackendDescription"}
			{fbvElement type="checkbox" name="trackPaymentSuccess" id="trackPaymentSuccess" checked=$trackPaymentSuccess label="plugins.generic.gtmSetup.manager.settings.trackPaymentSuccessDescription"}
		{/fbvFormSection}
	{/fbvFormArea}
